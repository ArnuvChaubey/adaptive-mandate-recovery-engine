...
policy recovers **+8.7% more mandates and +38.2% more value**, holds up across **19 of 19**
sensitivity scenarios and every scenario an LLM generated specifically to break it, captures
**95.0%** of the theoretical recovery ceiling — and **loses** on wasted attempts.
...
|---|---|---|---|---|---|
| recovery rate (recoverable) | 65.9% | 69.5% | **86.0%** | 81.8% | 90.6% *(ceiling)* |
| recovery rate (all mandates) | 59.7% | 63.0% | **77.9%** | 74.1% | 82.0% *(ceiling)* |
| value recovered | ₹19.28M | ₹25.34M | **₹30.13M** | ₹28.95M | ₹31.33M *(ceiling)* |
| wasted attempt rate | 0.8% | 0.8% | 2.1% | 1.4% | **0.0%** *(ceiling)* |
| median days to recovery | **1.0** | **1.0** | 3.0 | **1.0** | **1.0** |
| non-compliant proposals blocked | 11.7% | 0.0% | 0.0% | 0.0% | 0.0% |
...
question the table above can't: *is 86% good?* See [How far from the ceiling](#how-far-from-the-ceiling) below.
...
```
adaptive captures 95.0% of the oracle's recovery rate
...
```
recovery-rate lift positive in   19/19 scenarios   +6.4% to +50.2%   median +28.4%
value lift positive in           19/19 scenarios   +34.6% to +97.9%
wasted attempts IMPROVED in       1/19 scenarios

CONSERVATIVE HEADLINE (vs the strongest baseline):  +8.7% rate, +38.2% value
```

Then more scenarios generated by Claude with instructions to *break* it (see below): **none landed**.
The red team is LLM-generated and its output varies run to run, so re-run `python -m eval.redteam`
...
we were being conservative. Fixing it cut the lift from ~+36% to +11.8%. A second bug (entry 26: A6's
higher OTP ceiling was unreachable dead code, so the baseline was denied retries the law permits) cut
it again, from +12.5% to the conservative figure above. Both fixes strengthened the *comparison*, not
the candidate.
