...
|---|---|---|---|---|
| R1 | **Baseline retry interval/timing (highest risk item)** | Exact spacing between the 4 documented retry attempts, per instrument | **Re-checked 2026-08-28 and the earlier reading was wrong.** [Razorpay Payment Retries docs](https://razorpay.com/docs/payments/subscriptions/payment-retries/) state the full cadence for cards *and* UPI: "we automatically reattempt the charge on T+1 day... two more times on T+2 and T+3 days, respectively", then `halted`. Emandate — separate bank-confirmation model, plus the holiday-shift rule (T-1, or T-3 if T and T-1 are both holidays). | **HIGH — card/UPI multi-attempt spacing is a documented FACT, not an assumption.** The original "vague, not T+1/T+2/T+3" reading missed the section that spells it out. |
