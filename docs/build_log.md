...
## 35. The report a judge clicks was showing the numbers this project exists to have corrected

**What happened.** Pre-submission audit, checking the repo the way someone who will not run anything
would read it. `README.md` links twice to a committed snapshot — *"so it can be read without running
anything."* Its money figures:

| | committed snapshot | what the code produces today |
|---|---|---|
| baseline recovered | ₹9,389,075 | **₹19,275,486** |
| adaptive recovered | ₹21,303,601 | **₹30,125,537** |

The snapshot was from **2026-08-26**, before the baseline-rigging fix. It was showing a roughly **+127%
lift** — the inflated number produced by a baseline that had been accidentally crippled, which is the
single thing the build log, the README, and the demo video all describe correcting. The one artifact a
judge reads without running code was arguing the opposite of the project's whole thesis, in the
project's own voice.

**Also stale, and worse in a subtler way.** Every committed `.jsonl` sample predated hash chaining:

```
committed sample keys : [... policy_name, rule_description, ...]
current schema keys   : [... policy_name, prev_hash, record_hash, rule_description, ...]
```

Tamper-evidence is a headline capability and a beat in the video. A reader who took the README at its
word, clicked the sample to see the chain, and found no hash fields would reasonably conclude the
feature did not exist.

**Refreshed all six** committed artifacts from current outputs. For the `.jsonl` excerpts, the first
*n* records rather than an arbitrary slice — a middle slice's first `prev_hash` points at a record the
reader cannot see, so the excerpt would not verify standalone. Taken from the head, each sample is
independently checkable: `python -m audit.verify_chain eval/reports/examples/*.jsonl` → ALL CHAINS
INTACT.

**A real bug found by that same diff.** The webhook sample had no hash fields even after regeneration,
because `live_webhook_decisions.jsonl` itself had none. `webhook_receiver.py` was carrying **entry 31's
bug, unfixed**:

```python
_log = DecisionLog()              # fresh chain at GENESIS_HASH every process
_log.write_jsonl(AUDIT_PATH)      # mode "w" -- rewrites the whole file
```

Identical to the two lines fixed in `live_batch.py`. It survived because this path is started by hand
and its example artifact was old enough that the missing fields looked normal — *the staleness of the
artifact is what hid the bug in the code.* Fixed the same way (`load_chain_tip` seed, `append=True`),
with two regression tests pinning the wiring specifically, since the generic mechanism was already
tested and still did not catch this.

The webhook sample is now generated by replaying the **real captured Razorpay payloads** the pipeline
tests already use through `process_event` — the actual code path, offline.

**What it demonstrates.** Fixing a bug in one file does not fix it in the file that copied the pattern,
and a grep for the pattern is worth more than the memory of having fixed it. Also: committed artifacts
rot silently, and rot in the direction of whatever was true when you generated them — here, the exact
claim the project had spent three weeks disproving.
